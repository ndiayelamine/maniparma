<?php
// required headers
header("Access-Control-Allow-Origin: *");
header("Content-Type: application/json; charset=UTF-8");
 
// include database and object files
include_once '../config/core.php';
include_once '../shared/utilities.php';
include_once '../config/database.php';
include_once '../objects/chi_siamo.php';
 
// utilities
$utilities = new Utilities();
 
// instantiate database and chi_siamo object
$database = new Database();
$db = $database->getConnection();
 
// initialize object
$chi_siamo = new ChiSiamo($db);
 
// query chi_siamos
$stmt = $chi_siamo->readPaging($from_record_num, $records_per_page);
$num = $stmt->rowCount();
 
// check if more than 0 record found
if($num>0){
 
    // chi_siamos array
    $chi_siamos_arr=array();
    $chi_siamos_arr["records"]=array();
    $chi_siamos_arr["paging"]=array();
 
    // retrieve our table contents
    // fetch() is faster than fetchAll()
    // http://stackoverflow.com/questions/2770630/pdofetchall-vs-pdofetch-in-a-loop
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
        // extract row
        // this will make $row['name'] to
        // just $name only
        extract($row);
 
        $chi_siamo_item=array(
            "id" => $id,
            "column_position" => $column_position,
            "column_title" => html_entity_decode($column_title),
            "column_content" => $column_content,
            "img_url_big" => $img_url_big
        );
 
        array_push($chi_siamos_arr["records"], $chi_siamo_item);
    }
 
 
    // include paging
    $total_rows=$chi_siamo->count();
    $page_url="{$home_url}chi_siamo/read_paging.php?";
    $paging=$utilities->getPaging($page, $total_rows, $records_per_page, $page_url);
    $chi_siamos_arr["paging"]=$paging;
 
    // set response code - 200 OK
    http_response_code(200);
 
    // make it json format
    echo json_encode($chi_siamos_arr);
}
 
else{
 
    // set response code - 404 Not found
    http_response_code(404);
 
    // tell the user chi_siamos does not exist
    echo json_encode(
        array("message" => "No chi_siamos found.")
    );
}
?>